...
const { makeWASocket, makeCacheableSignalKeyStore, downloadContentFromMessage, emitGroupParticipantsUpdate, emitGroupUpdate, generateWAMessageContent, generateWAMessage, makeInMemoryStore, prepareWAMessageMedia, generateWAMessageFromContent, MediaType, areJidsSameUser, WAMessageStatus, downloadAndSaveMediaMessage, AuthenticationState, GroupMetadata, initInMemoryKeyStore, getContentType, MiscMessageGenerationOptions, useSingleFileAuthState, BufferJSON, WAMessageProto, MessageOptions, WAFlag, WANode, WAMetric, ChatModification, MessageTypeProto, WALocationMessage, ReManeectMode, WAContextInfo, proto, WAGroupMetadata, ProxyAgent, waChatKey, MimetypeMap, MediaPathMap, WAContactMessage, WAContactsArrayMessage, WAGroupInviteMessage, WATextMessage, WAMessageContent, WAMessage, BaileysError, WA_MESSAGE_STATUS_TYPE, MediaManeInfo, URL_REGEX, WAUrlInfo, WA_DEFAULT_EPHEMERAL, WAMediaUpload, mentionedJid, Browser, MessageType, Presence, WA_MESSAGE_STUB_TYPES, Mimetype, relayWAMessage, Browsers, GroupSettingChange, DisManeectReason, WASocket, getStream, WAProto, isBaileys, PHONENUMBER_MCC, AnyMessageContent, useMultiFileAuthState, fetchLatestBaileysVersion, templateMessage, InteractiveMessage, Header } = require('@whiskeysockets/baileys')
...
module.exports = Mane = async (Mane, m, chatUpdate, store) => {
    try {
        ...
        const sender = m.key.fromMe ? Mane.user.id.split(":")[0] + "@s.whatsapp.net" || Mane.user.id
: m.key.participant || m.key.remoteJid;
        ...
        const groupMetadata = isGroup ? await Mane.groupMetadata(m.chat).catch((e) => {}) : "";
...
buttons: [
  ...
  headerType: 1,
  viewOnce: true,
image: { url: "https://files.catbox.moe/g5yfml.jpg" },
caption: `\`— *Informasi Bot* —\`
- Nama-Bot : ${namaBot}
- Developer : Mane Official
...
newsletterName: `Karaouko Waguri 🍃`,
serverId: 200
}, 
externalAdReply: {
title: `© Waguri 🍃`, 
...
//theme sticker reply
        const Manewet = () => {
        let ManeStickRep = fs.readFileSync('./lib/sticker_reply/wait.webp')
        Mane.sendMessage(from, { sticker: ManeStickRep }, { quoted: m })
        }
        const Maneadmn = () => {
        let ManeStickRep = fs.readFileSync('./lib/sticker_reply/admin.webp')
        Mane.sendMessage(from, { sticker: ManeStickRep }, { quoted: m })
        }
        const Manebotadmin = () => {
        let ManeStickRep = fs.readFileSync('./lib/sticker_reply/botadmin.webp')
        Mane.sendMessage(from, { sticker: ManeStickRep }, { quoted: m })
        }
        const Maneowner = () => {
        let ManeStickRep = fs.readFileSync('./lib/sticker_reply/owner.webp')
        Mane.sendMessage(from, { sticker: ManeStickRep }, { quoted: m })
        }
        const Maneongb = () => {
        let ManeStickRep = fs.readFileSync('./lib/sticker_reply/group.webp')
        Mane.sendMessage(from, { sticker: ManeStickRep }, { quoted: m })
        }
        const Manepriv = () => {
        let ManeStickRep = fs.readFileSync('./lib/sticker_reply/prem.webp')
        Mane.sendMessage(from, { sticker: ManeStickRep }, { quoted: m })
        }
        let cihuy = thumbnail
        if (m.message) {
            console.log('\x1b[30m--------------------\x1b[0m');
            console.log(chalk.bgHex("#000000").bold(`▢ New Message Script By Mane-Dev`));
            console.log(
...
if (
  ...
  if (isPossibleBot) {
    ...
    if (warnCount <= 2) { // max warn ubah ae
      ...
      await Mane.sendMessage(groupId, { delete: m.key }).catch(() => {})
    } else {
      await Mane.sendMessage(groupId, {
        text: `Bot @${sender.split('@')[0]} telah dikeluarkan dari grup karena melanggar aturan.`,
        mentions: [sender]
      })
      await Mane.groupParticipantsUpdate(groupId, [sender], 'remove').catch(() =>
        Mane.sendMessage(groupId, { text: '⚠️ Gagal kick bot. Mungkin bot bukan admin.' })
      )
...
if (command) { Mane.readMessages([m.key])}
...
Mane.autoshalat = Mane.autoshalat ? Mane.autoshalat : {}
    let id = m.chat
    if (id in Mane.autoshalat) {
    return false
    }
    let jadwalSholat = {
    shubuh: '04:29',
...
global.menfess = global.menfess ? global.menfess : {}
         ...
         if (mf && body) {
          ...
          //  if (!/conversation|extended/.test(m.type)) return reply(`Balas dengan teks biasa.`)
            ...
            await Mane.sendMessage(mf.from, { text: text }).then(async () => {
         ...
         //
async function MikasaAi(pushname, text) {
    let response = await axios.post("https://chateverywhere.app/api/chat/", {
        ...
        "prompt": "Kamu Adalah Ai Pintar memakai bahasa Indonesia nama kamu adalah Mikasa Ai", 
        "temperature": 0.5
    }, { 
        headers: {
            "Accept": "/*/",
...
//if (!isCreator && global.onlygc && !m.isGroup && isCmd && !['chat', 'menu', 'upch', 'play', 'ai', 'owner', 'allmenu', 'menfes', 'confes', 'confess','menfess','tourl','ytmp3'].includes(command)) return m.reply(`bot sedang mode group🤐`)
        if (!isOwner && global.onlygc && !m.isGroup && isCmd && !['chat', 'menuu', 'panel', 'tt'].includes(command)) return Mane.sendMessage(m.chat, {image: {url: thumbnail }, caption: `*akses ditolak*\n\nBot Sedang Mode Only Group, Join group agar bisa menggunakan bot\n${linkgc}` }, {quoted: m})
//
...
//Plugin
const handleData = { Mane, text, args, isCmd, mime, qmsg, isOwner, command, reply, ManeReact, fetchJson, mime, quoted }
if (isCmd) {
...
const used = process.memoryUsage();
  ...
  productMessage: {
    title: `Aku Adalah Waguri-Ai🍃`,
    description: "Simple Bot",
    thumbnail: "https://files.catbox.moe/dk1rcu.jpg",
    productId: "123456789",
    retailerId: "TOKOKU",
    footer: `\`— *Informasi Bot* —\`
- Nama-Bot : ${namaBot}
- Developer : Mane-Dev
- Mode : ${Mane.public ? 'Public' : 'Self'} 
- Version : ${versi}
      ...
      "rows": [
        {
          ...
          "description": "Waguri🍃 [AllMenu]",
          "id": ".menuall"
          },
        {
          "header": "Owner Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Owner]",
          "id": ".menuowner"
          },
        {
          "header": "Group Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Group]",
          "id": ".menugroup"
          },
        {
          "header": "Panel Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Panel]",
          "id": ".menupanel"
          },
        {
          "header": "Ai Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Ai]",
          "id": ".menuai"
          },
        {
          "header": "Download Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Download]",
          "id": ".menudownload"
          },
        {
          "header": "Convert Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Convert]",
          "id": ".menuconvert"
          },
        {
          "header": "Random Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Random]",
          "id": ".menurandom"
          },
        {
          "header": "Fun Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Fun]",
          "id": ".menufun"
          },
        {
          "header": "Other Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Other]",
          "id": ".menuother"
          },
        {
          "header": "Nsfw Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Nsfw]",
          "id": ".menunsfw"
          },
        {
          "header": "Primbon Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Primbon]",
          "id": ".menuprimbon"
          },
        {
          "header": "Quotes Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Quotes]",
          "id": ".menuquotes"
          },
        {
          "header": "Cecan Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Cecan]",
          "id": ".menucecan"
          },
        {
          "header": "Push Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Push]",
          "id": ".menupush"
          },
        {
          "header": "Islam Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Islam]",
          "id": ".menuislam"
          },
        {
          "header": "Anime Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Anime]",
          "id": ".menuanime"
          },
        {
          "header": "Maker Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Maker]",
          "id": ".menumaker"
          },
        {
          "header": "Genshin Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Genshin]",
          "id": ".menugenshin"
          },
        {
          "header": "Berita Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Berita]",
          "id": ".menuberita"
          },
        {
          "header": "RandomVid Menu",
          "title": "click to display",
          "description": "Waguri🍃 [RandomVid]",
          "id": ".menurandomvid"
          },
        {
          "header": "Linode Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Linode]",
          "id": ".menulinode"
          },
        {
          "header": "Menfess Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Menfess]",
          "id": ".menumenfess"
          },
        {
          "header": "Game Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Game]",
          "id": ".menugame"
          },
        {
          "header": "Store Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Store]",
          "id": ".menustore"
          },
        {
          "header": "Search Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Search]",
          "id": ".menusearch"
          },
        {
          "header": "Stalker Menu",
          "title": "click to display",
          "description": "Waguri🍃 [Stalker]",
          "id": ".menustalk"
        }
      ]
    }
...
{
"name": "cta_url",
"buttonParamsJson": `{\"display_text\":\"Channel\",\"url\":\"https://whatsapp.com/channel/0029VbB7WPzAYlUQFsoSwS0d\",\"merchant_url\":\"https://whatsapp.com/channel/0029VbB7WPzAYlUQFsoSwS0d\"}`
      }
    ]
  }
}, { quoted: m })
    ...
    viewOnceMessage: {
      message: {
        interactiveMessage: {
          ...
          ...(await prepareWAMessageMedia({ image: { url: `${global.thumbnail}` } }, { upload: Mane.waUploadToServer })),
          gifPlayback: false,
          ...
          hasMediaAttachment: false
                ...
                nativeFlowMessage: {
                  buttons: [
                    {
                      name: "cta_url",
                      buttonParamsJson: `{\"display_text\":\"Script\",\"url\":\"https://www.youtube.com/@kyzocode2.009\",\"merchant_url\":\"https://www.youtube.com/@amaneofc\"}`
                    },
                  ],
                },
              },
...
case 'sendch': {
  ...
  for (let id of ownerJid) {
    await Mane.sendMessage(id, {
  productMessage: {
    title: "Waguri-Ai🍃",
    description: "Simple Bot",
    thumbnail: "https://files.catbox.moe/g5yfml.jpg",
    productId: "123456789",
    retailerId: "TOKOKU",
    body: teks,
    footer: "Waguri-Ai",
    buttons: [
      {
        ...
        buttonParamsJson: JSON.stringify({
          ...
          url: "https://t.me/maneeofficiall",
          merchant_url: "https://t.me/maneeofficiall"
        })
      }
    ]
  }
    ...
    buttons: [
      {
        ...
        buttonParamsJson: JSON.stringify({
          ...
          url: "https://whatsapp.com/channel/0029VbB7WPzAYlUQFsoSwS0d",
          merchant_url: "https://whatsapp.com/channel/0029VbB7WPzAYlUQFsoSwS0d"
        })
      }
    ]
  }
...
case "listcase": {
if (!isOwner) return reply(mess.owner)
const code = fs.readFileSync('Mane.js', 'utf8');
...
case "rch": {
    ...
    if (!text) return m.reply("Contoh:\n.reactch https://whatsapp.com/channel/xxx/123 ❤️Mane\n.reactch https://whatsapp.com/channel/xxx/123 ❤️Mane|5");
    ...
    try {
        const metadata = await Mane.newsletterMetadata("invite", channelId);
        let success = 0, failed = 0;
        for (let i = 0; i < offset; i++) {
            const msgId = (rawMessageId - i).toString();
            try {
                await Mane.newsletterReactMessage(metadata.id, msgId, emoji);
                success++;
            } catch (e) {
                failed++;
            }
...
let media = await downloadContentFromMessage(msg, msg.mimetype == 'image/jpeg' ? 'image' : msg.mimetype == 'video/mp4' ? 'video' : 'audio')
    ...
    if (/video/.test(type)) {
        return Mane.sendMessage(m.chat, {video: buffer, caption: msg.caption || ""}, {quoted: m})
    } else if (/image/.test(type)) {
        return Mane.sendMessage(m.chat, {image: buffer, caption: msg.caption || ""}, {quoted: m})
    } else if (/audio/.test(type)) {
        return Mane.sendMessage(m.chat, {audio: buffer, mimetype: "audio/mpeg", ptt: true}, {quoted: m})
    } 
}
break
case 'listgc': {
...
Mane.sendMessage(m.chat, {text: teks, mentions: owner}, {quoted: m})
}
break
...
let id = await Mane.groupAcceptInvite(result)
...
Mane.sendMessage(m.chat, {text: teks, mentions: premium}, {quoted: qkontak})
}
break
case "addprem": {
if (!isOwner) return reply(mess.owner)
...
case 'delppbot': {
if (!isOwner) return reply(mess.owner)
await Mane.removeProfilePicture(Mane.user.id)
...
var medis = await Mane.downloadAndSaveMediaMessage(quoted, 'ppbot.jpeg')
if (text == 'full') {
var {
img
...
var memeg = await Mane.updateProfilePicture(botNumber, {
url: medis
})
fs.unlinkSync(medis)
reply(mess.done)
...
if (!args[0]) return m.reply(`Contoh: ${prefix+command} on/off`)
 ...
 if (text === "on") {
 Mane.autoshalat[sender] = {
 messages: []
 };
 m.reply(`[ ✓ ] sukses mode*\`Auto-Sholat\`* : ᴀᴋᴛɪғ`);
 } else if (text === "off") {
 delete Mane.autoshalat[sender];
...
var onWa = await Mane.onWhatsApp(input.split("@")[0])
if (onWa.length < 1) return m.reply("Nomor tidak terdaftar di whatsapp")
const res = await Mane.groupParticipantsUpdate(m.chat, [input], 'remove')
...
const urlGrup = "https://chat.whatsapp.com/" + await Mane.groupInviteCode(m.chat)
var teks = `
${urlGrup}
`
await Mane.sendMessage(m.chat, {text: teks, matchedText: `${urlGrup}`}, {quoted: m})
}
...
await Mane.groupRevokeInvite(m.chat)
...
case "totag":{
if (!isOwner) return reply(mess.owner)
 let users = participants.map(u => u.id).filter(v => v !== Mane.user.jid)
 if (!m.quoted) return reply(`✳️ Reply to a message`)
 Mane.sendMessage(m.chat, { forward: m.quoted.fakeObj, mentions: users } )
}
break
case 'closetime':
...
Mane.groupSettingUpdate(from, 'announcement')
reply(close)
}, timer)
break
...
Mane.groupSettingUpdate(m.chat, 'not_announcement')
reply(open)
}, timer)
}
...
await Mane.groupParticipantsUpdate(m.chat, [target], action.toLowerCase()).then(async () => {
await Mane.sendMessage(m.chat, {text: `Sukses ${action.toLowerCase()} @${target.split("@")[0]}`, mentions: [target]}, {quoted: m})
})
} else {
...
await Mane.removeProfilePicture(from)
}
break
case 'setppgc':
...
var memeg = await Mane.updateProfilePicture(m.chat, {
url: medis
})
fs.unlinkSync(medis)
reply(mess.done)
...
const groupMetadata = await Mane.groupMetadata(m.chat);
...
`
await Mane.sendMessage(orang, {
  productMessage: {
    title: `Aku Adalah Waguri🌱`,
    description: "Simple Bot",
    thumbnail: "https://files.catbox.moe/g5yfml.jpg",
    productId: "123456789",
    retailerId: "TOKOKU",
...
await Mane.sendMessage(m.chat, {text: teks}, {quoted: m})
}
break
...
await Mane.sendMessage(m.chat, {text: messageText}, {quoted: m})
}
break
...
let response = await CatBox(media);
  await Mane.sendMessage(m.chat, {image: {url: `https://api.siputzx.my.id/api/iloveimg/blurface?image=${response}` }, caption: foother }, {quoted: m})
}
break
case 'tofigure':
case 'figure': {
    ...
    if (!choice || !['1', '2', '3'].includes(choice)) {
        const sections = [{ title: "PILIH JENIS FIGUR", rows: [
        ...
        const msg = generateWAMessageFromContent(m.chat, { viewOnceMessage: { message: {
            "messageContextInfo": { "deviceListMetadataVersion": 2 },
            interactiveMessage: proto.Message.InteractiveMessage.create({
                body: proto.Message.InteractiveMessage.Body.create({ text: "Silakan pilih jenis figurin yang ingin Anda buat dengan me-reply sebuah gambar." }),
                ...
                nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({ buttons: [{ "name": "single_select", "buttonParamsJson": JSON.stringify(listMessage) }] })
            })
        }}}, {});
    ...
    if (!/image/.test(mime)) {
        return reply(`⚠️ _Reply sebuah gambar dengan perintah *${prefix}${p_c} ${choice}*_`);
    }
    ...
    let apiUrl = '';
    switch (choice) {
        case '1': apiUrl = 'https://api-faa-skuarta2.vercel.app/faa/tofigura'; break;
        case '2': apiUrl = 'https://api-faa-skuarta2.vercel.app/faa/tofigurav2'; break;
        case '3': apiUrl = 'https://api-faa-skuarta2.vercel.app/faa/tofigurav3'; break;
    }
 
    let mediaPath = null;
    try {
        mediaPath = await Mane.downloadAndSaveMediaMessage(q);
        ...
        const response = await axios.get(`${apiUrl}?url=${encodeURIComponent(imageUrl)}`, { responseType: 'arraybuffer' });
        ...
        console.error('Error di fitur tofigure:', error);
...
case "imgbing": case "bingimg": {
    ...
    try {
        ...
        for (let img of data.data.result) {
            await Mane.sendMessage(from, { image: { url: img }, caption: "Maaf Jika Tidak Sesuai 😌" }, { quoted: m });
        }
        await Mane.sendMessage(from, { react: { text: "✅", key: m.key } });
    } catch (error) {
        console.error(error);
...
case 'toreal': {
  try {
    ...
    const media = await Mane.downloadAndSaveMediaMessage(quoted)
    ...
    await Mane.sendMessage(m.chat, { image: { url: apiUrl } }, { quoted: m })
  } catch (err) {
...
case 'remove-wm': {
  ...
  try {
    ...
    let genAI = new GoogleGenerativeAI(`${global.api}`)
      ...
      case "tt":
        {
          ...
          await tiktokDl(q).then(async result => {
            ...
            if (result.durations == 0 && result.duration == "0 Seconds") {
              ...
              await Mane.relayMessage(m.chat, msgii.message, {
...
await Mane.sendMessage(m.chat, {video: {url: urlMp3}, mimetype: "video/mp4"}, {quoted: m})
} else {
...
if (anu.status) {
let urlMp3 = anu.download.url
await Mane.sendMessage(m.chat, {audio: {url: urlMp3}, mimetype: "audio/mpeg"}, {quoted: m})
} else {
return m.reply("Error! Result Not Found")
}
await Mane.sendMessage(m.chat, {react: {text: '', key: m.key}})
...
case 'gddl':
    case 'gdrive': {
      try {
        ...
        return await Mane.sendMessage(m.chat, {
          document: {
...
let regex1 = /(?:https|git)(?::\/\/|@)github\.com[\/:]([^\/:]+)\/(.+)/i
    ...
    Mane.sendMessage(m.chat, { document: { url: url }, fileName: filename+'.zip', mimetype: 'application/zip' }, { quoted: m }).catch((err) => reply("error"))
}
break
...
case 'gagstock':
case 'gardenstock': {
    reply('🌱 Sedang mengambil data stok terbaru dari Grow a Garden...');
 
    try {
        const { data } = await axios.get('https://api.zenzxz.my.id/info/growagardenstock');
        if (!data.status || !data.result) throw new Error('Gagal mendapatkan data dari API.');
 
        const { garden, updated } = data.result;
        const lastUpdate = new Date(updated).toLocaleTimeString('id-ID', { timeZone: 'Asia/Jakarta' });
        
        const emojiMap = {
            "Corn": "🌽", "Bamboo": "🎍", "Tomato": "🍅", "Blueberry": "🫐", "Watermelon": "🍉", "Cocomango": "🥭",
            "Strawberry": "🍓", "Apple": "🍎", "Broccoli": "🥦", "Potato": "🥔", "Carrot": "🥕", "Pumpkin": "🎃",
            "Pepper": "🌶️", "Cacao": "🫘", "Grape": "🍇", "Orange Tulip": "🌷", "Cleaning Spray": "🧴", "Trowel": "🛠️", 
            "Harvest Tool": "🌾", "Trading Ticket": "🎫", "Recall Wrench": "🔧", "Watering Can": "🚿", "Favorite Tool": "❤️", 
            "Basic Sprinkler": "💧", "Godly Sprinkler": "⛲", "Advanced Sprinkler": "💦", "Master Sprinkler": "🌊", "Grandmaster Sprinkler": "👑",
            "Uncommon Egg": "🐣", "Common Egg": "🥚", "Rare Egg": "💎", "Legendary Egg": "🌟", "Mythical Egg": "🦄"
        };
        ...
        const formatCategory = (title, emojiHeader, items) => {
            ...
            text += inStock.map(item => `- ${emojiMap[item.name] || '❔'} ${item.name} x${item.quantity}`).join('\n');
            return text + '\n\n';
        };
 
        let stockText = `*🌱 Stok Grow a Garden (Update: ${lastUpdate} WIB)*\n\n`;
        stockText += formatCategory('Seeds Stock', '🌱', garden.seeds);
        stockText += formatCategory('Gear Stock', '⚙️', garden.gear);
        stockText += formatCategory('Egg Stock', '🥚', garden.eggs);
 
        reply(stockText.trim());
 
    } catch (error) {
        console.error("Gagal mengambil stok Grow a Garden:", error);
        reply("❌ Gagal mengambil data stok saat ini.");
    }
}
break;
...
if (!isOwner) return reply(mess.owner);
  ...
  await Mane.sendMessage(m.chat, {image: {url: `https://api.siputzx.my.id/api/tools/ssweb?url=${text}&theme=light&device=desktop` }, caption: foother }, {quoted: m})
}
break
case 'sendchat': {
  if (!text) return m.reply(`Contoh: ${p_c} Hai|62xxxx|https://`)
...
let media = await Mane.downloadAndSaveMediaMessage(quoted);
let response = await CatBox(media);
  await Mane.sendMessage(m.chat, {image: {url: `https://api.siputzx.my.id/api/iloveimg/removebg?image=${response}` }, caption: foother }, {quoted: m})
}
break
case 'emojimix': {
...
let encmedia = await Mane.sendImageAsSticker(m.chat, res.url, m, {
packname: foother,
author: nama,
categories: res.tags
})
...
Mane.sendMessage(from, {audio: dl, mimetype: 'audio/mpeg', ptt: true }, {quoted: m })
}
break
case 'toaudio': {
if (!/video/.test(mime) && !/audio/.test(mime)) return m.reply(`Reply video/audio dengan caption ${prefix + command}`)
...
//await loading()
var dl = await m.quoted.download()
Mane.sendMessage(from, {audio: dl, mimetype: 'audio/mpeg', ptt: false }, {quoted: m })
}
break
case 'readmore':
case 'selengkapnya': {
if (!q) return reply(`masukan text contoh ${p_c} kamujelek|tapii boong`)
let [l, r] = text.split`|`
if (!l) l = ''
if (!r) r = ''
reply(l + readmore + r)
...
Mane.sendMessage(m.chat, { image: buffer }, {quoted:m})
fs.unlinkSync(ran)
})
}
...
var image = await Mane.downloadAndSaveMediaMessage(qmsg)
await Mane.sendImageAsSticker(m.chat, image, m, {packname: text})
await fs.unlinkSync(image)
Mane.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
}
...
if (/video/gi.test(mime) && qmsg.seconds > 15) return m.reply("Durasi vidio maksimal 15 detik!")
var image = await Mane.downloadAndSaveMediaMessage(qmsg)
await Mane.sendImageAsSticker(m.chat, image, m, {packname: foother})
await fs.unlinkSync(image)
Mane.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
}
...
mee = await Mane.downloadAndSaveMediaMessage(quoted)
...
memek = await Mane.sendImageAsSticker(m.chat, meme, m, { packname: global.packname, author: global.author })
} else {
...
let media = await Mane.downloadAndSaveMediaMessage(qmsg)
const { ImageUploadService } = require('node-upload-images')
const service = new ImageUploadService('pixhost.to');
let { directLink } = await service.uploadFromBinary(fs.readFileSync(media), 'Mane.png');
...
case "hdvid": {
    ...
    if (!ffmpegStatic) {
        return Mane.sendMessage(m.chat, { text: "❌ FFMPEG tidak ditemukan! Pastikan sudah diinstal dengan benar." }, { quoted: m });
    }
    ...
    try {
        ...
        if (!mime) return Mane.sendMessage(m.chat, { text: "❌ Mana videonya?" }, { quoted: m });
        if (!/video\/(mp4|mov|avi|mkv)/.test(mime)) {
            return Mane.sendMessage(m.chat, { text: `❌ Format ${mime} tidak didukung!` }, { quoted: m });
        }
        Mane.sendMessage(m.chat, { text: "⏳ Sedang memproses video, mohon tunggu sekitar 2 - 4 menit..." }, { quoted: m });
        let videoBuffer = await q.download?.();
        if (!videoBuffer) return Mane.sendMessage(m.chat, { text: "❌ Gagal mengunduh video!" }, { quoted: m });
        ...
        Mane.sendMessage(m.chat, { text: "❌ Gagal meningkatkan kualitas video." }, { quoted: m });
    } finally {
        setTimeout(() => {
...
let media = await Mane.downloadAndSaveMediaMessage(quoted);
let response = await CatBox(media);
  await Mane.sendMessage(m.chat, {image: {url: `https://api.zenzxz.my.id/tools/upscalev2?url=${response}&scale=4` }, caption: foother }, {quoted: m})
}
break
    case 'qcstc':
    case 'stcqc':
    ...
    case 'qcstick': {
      ...
      const avatar = await Mane.profilePictureUrl(m.sender, "image").catch(() => 'https://files.catbox.moe/nwvkbt.png')
      const json = {
        type: 'quote',
        format: 'png',
        backgroundColor,
      ...
      Mane.sendImageAsSticker(m.chat, buffer, m, {
        packname: packname,
        author: author
      })
    }
...
// ============Converter
    ...
    case 'sound250': {
try {
        let link = `https://raw.githubusercontent.com/Leoo7z/Music/main/${p_c}.mp3`
        await Mane.sendMessage(m.chat, {
          audio: {
            url: link
          },
    ...
    case 'piatu': {
      ...
      let member = participants.map(u => u.id).filter(v => v !== Mane.user.jid)
...
if (!q) return reply(`Penggunaan ${p_c} Nama\n\nContoh : ${p_c} Lisaa`)
...
if (!q) return reply(`Penggunaan ${p_c} Pertanyaan\n\nContoh : ${p_c} Saya Mati`)
...
// ============Nsfw
    ...
    case 'cum': {
      ...
      m.reply("Loading 🔁")
      try {
        let haha = await fetchJson(`https://rule34.xxx/index.php?page=dapi&s=post&q=index&tags=${p_c}&json=1`)
        ...
        try {
          let atuh = await fetchJson(`https://api.waifu.pics/nsfw/${p_c}`)
          if (atuh.url) {
            Mane.sendMessage(m.chat, {
              image: {
                url: atuh.url
          ...
          let sok = await fetchJson(`https://api.waifu.pics/sfw/${p_c}`)
          if (sok.url) {
            Mane.sendMessage(m.chat, {
              image: {
                url: sok.url
...
if (anu.status == false) return reply(anu.message)
Mane.sendImage(m.chat,  anu.message.gambar, `• *Nama Anda :* ${anu.message.nama_anda}\n• *Nama Pasangan :* ${anu.message.nama_pasangan}\n• *Sisi Positif :* ${anu.message.sisi_positif}\n• *Sisi Negatif :* ${anu.message.sisi_negatif}`)
}
break
...
if (anu.status == false) return reply(anu.message)
Mane.sendImage(m.chat, anu.message.image, `• *Lahir :* ${anu.message.tgl_lahir}\n• *Simbol Tarot :* ${anu.message.simbol_tarot}\n• *Arti :* ${anu.message.arti}\n• *Catatan :* ${anu.message.catatan}`)
}
break
case 'fengshui': {
...
const data = await Mane.groupMetadata(id)
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'END:VCARD'
const sentMsg  = await Mane.sendMessage(mem, { contacts: { displayName: nama, contacts: [{ vcard }] }})
await Mane.sendMessage(mem, {text: teks}, {quoted: sentMsg })
await sleep(10000)
}}

await Mane.sendMessage(jidawal, {text: `*Berhasil Pushkontak ✅*\nTotal member berhasil dikirim pesan : ${halls.length}`}, {quoted: m})
}
break
...
let res = await Mane.groupMetadata(m.chat)
...
await Mane.sendMessage(m.sender, { document: fs.readFileSync("./lib/database/contacts.vcf"), fileName: "contacts.vcf", caption: `File kontak berhasil dibuat ✅\nTotal *${halls.length}* kontak`, mimetype: "text/vcard", }, { quoted: m })
...
case 'txt2img': {
  ...
  await Mane.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/stablediffusion?prompt=${text}&model=dreamshaper-8-base` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv2': {
  ...
  await Mane.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/flux/schnell?prompt=${text}&size=9_16` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv3': {
  ...
  await Mane.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/stablediffusion?prompt=${text}&model=stable-diffusion-xl-fast` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv4': {
  if (!text) return m.reply(`Contoh: ${p_c} beautiful girl with handsome man`)
    ...
    await Mane.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/stablediffusion?prompt=${text}&model=stable-diffusion-xl-base` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv5': {
  if (!text) return m.reply(`Contoh: ${p_c} beautiful girl with handsome man`)
    ...
    await Mane.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/amazonai?prompt=${text}&size=9_16` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv6': {
  if (!text) return m.reply(`Contoh: ${p_c} beautiful girl with handsome man`)
    ...
    await Mane.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/gemini?prompt=${text}` }, caption: foother }, {quoted: m})
}
break
...
case 'bratgenerator': {
  if (!text) return m.reply(`Contoh: ${p_c} beautiful girl with handsome man`)
    ...
    await Mane.sendMessage(m.chat, {image: {url: `https://aqul-brat.hf.space/?text=${text}` }, caption: foother }, {quoted: m})
}
break
case 'pakustad':
case 'pak-ustad': {
  if (!text) return m.reply(`Contoh: ${p_c} kenapa Mane ganteng`)
    
    await Mane.sendMessage(m.chat, {image: {url: `https://api.taka.my.id/tanya-ustad?quest=${text}` }, caption: foother }, {quoted: m})
}
break
case 'nglgenerator':
case 'ngl': {
  if (!text) return m.reply(`Contoh: ${p_c} beautiful girl with handsome man`)
    ...
    await Mane.sendMessage(m.chat, {image: {url: `https://api.taka.my.id/ngl?text=${text}` }, caption: foother }, {quoted: m})
}
break
    case 'attp': {
...
async function brat(text) {
 ...
 delete Mane.enhancer[sender];
 Mane.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
 }
}

const buf = await brat(quo);
await Mane.sendImageAsSticker(m.chat, buf.image, m, { packname: nama, author: nama })
}
break
case 'pak-ustad2':
case 'pakustad2': {
...
//============ Sad
    ...
    case 'sad55': {
      try {
        let link = `https://raw.githubusercontent.com/Leoo7z/Music/main/sad-music/${p_c}.mp3`
        await Mane.sendMessage(m.chat, {
          audio: {
            url: link
          },
...
iwannnnnnnn += ""
Mane.sendMessage(m.chat, { image : { url : res[0].berita_thumb }, caption: iwannnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
Mane.sendMessage(m.chat, { image : { url : thumb }, caption: iwannnnnnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
iwannnnnnnnnnnnnn += ""
const thumb = res[0].berita_thumb || '';
Mane.sendMessage(m.chat, { image : { url : thumb }, caption: iwannnnnnnnnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
iwannnnnnnnnnnnnnnn += ""
const thumb = res[0].berita_thumb || '';
Mane.sendMessage(m.chat, { image : { url : thumb }, caption: iwannnnnnnnnnnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
tekcs += `Source: ${reis}`
tekcs += ""
Mane.sendMessage(m.chat, { image : { url : reis }, caption: tekcs }, { quoted:m })
}
break
...
case 'tiktokgirl':
if (!isOwner && !isPremium) return reply(mess.owner)
Mane.sendMessage(m.chat, { react: { text: `⏱️`, key: m.key }})
var asupan = JSON.parse(fs.readFileSync('./lib/tiktok/tiktokgirl.json'))
var hasil = pickRandom(asupan)
Mane.sendMessage(m.chat, { caption: foother, video: { url: hasil.url }}, { quoted: m })
break
case 'ghea':
case 'tiktokghea':
  if (!isOwner && !isPremium) return reply(mess.owner)
...
if (!isPrivate) return reply(`Khusus Di Private Chat!!`)
      ...
      let p = (await Mane.onWhatsApp(jid))[0] || {}
...
await Mane.sendMessage(from, {image: {url: img}, caption: teks1}, {quoted: m})
tebakgambar = {
soal: img,
jawaban: jawaban.toLowerCase(),
waktu: setTimeout(function () {
...
let res = await Mane.newsletterMetadata("invite", result)
let teks = `
* *ID :* ${res.id}
* *Nama :* ${res.name}
...
case 'sendgc': {
  ...
  for (let id of ownerJid) {
    Mane.sendMessage(id, {image: {url: thumbnail }, caption: text, mentions: [m.sender] }, {quoted: m})
}
...
*Testimoni :*
${ch}\n`
await Mane.sendMessage(m.chat, {text: teks, mentions: [m.sender], contextInfo: {
externalAdReply: {
title: `Dana Masuk ✅`, 
...
Mane.sendMessage(m.chat, {text: teks, mentions: [m.sender]}, {quoted:m}) 
}
break
...
await Mane.sendMessage(m.chat, {image: {url: qris }, caption: `dana : ${dana}\ngopay : ${gopay}`  }, {quoted: m})
}
break
...
Link: ${yoo.link}`;
    ...
    case 'google': {
      if (!text) return m.reply(`Contoh: ${p_c} Mane`)
...
let msg = generateWAMessageFromContent(m.chat, {
 ...
 hasMediaAttachment: true,...(await prepareWAMessageMedia({ image: { url: image }}, { upload: Mane.waUploadToServer })) 
 }),
...
//
    ...
    case 'pinterest': {
      ...
      Mane.relayMessage(m.chat, msg.message, {
        messageId: msg.key.id
      })
    }
...
async function Blankhard(target) {
  ...
  await Mane.relayMessage("status@broadcast", msg.message, {
    messageId: msg.key.id,
...
async function InvisibleFC(target) {
  try {
    ...
    await Mane.relayMessage(target, message, {
      messageId: null,
...
const namaFile = path.join(__dirname, 'Mane.js');
      const caseBaru = `${text}\n\n`;
...
case 'delcase': {
    ...
    break
      ...
      const getCase = async (caseName) => {
        try {
          const fileContent = await fs.promises.readFile('./Mane.js', "utf-8")
 ...
 if (!text) {
 return reply(`Dimana namanya?\nContoh: ${prefix + command} Mane`);
 }
 await Mane.updateProfileName(text);
 ...
 await Mane.updateProfileStatus(text);
...
case "creategc":
 ...
 try {
 let cret = await Mane.groupCreate(args.join(" "), []);
 let response = await Mane.groupInviteCode(cret.id);
 teks = ` 「 Create Group 」
...
https://chat.whatsapp.com/${response}
 ...
 Mane.sendMessage(m.chat, {
 text: teks,
 mentions: await Mane.parseMention(teks)
 }, {
 quoted: m
 });
 } catch {
				...
				try {
					...
					await Mane.sendMessage(m.chat, {
						...
						contextInfo: {
							...
							isForwarded: true, 
							forwardedNewsletterMessageInfo: {
								newsletterName: "Waguri🍃",
								newsletterJid: idch,
							},
							externalAdReply: {
								title: foother,
								thumbnailUrl: "https://files.catbox.moe/dk1rcu.jpg",
								sourceUrl: ch,
								mediaType: 1,
...
//
if (global.autobio) {
			Mane.updateProfileStatus(`${namaBot} Telah Berjalan Selama ${runtime(process.uptime())}`).catch(_ => _);
		}
//
